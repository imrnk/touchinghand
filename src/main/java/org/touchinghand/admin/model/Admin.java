...
public class Admin extends User implements Serializable{
 ...
 //   authorities = roles.stream().map(r -> new SimpleGrantedAuthority(r.getRole())).collect(Collectors.toList()); 
 // }
  
  private void buildRoleFromGrantedAuthorities(List<GrantedAuthority> authorities){
    roles = authorities.stream().map(ga -> 
                                   new Authority(this.userName, ga.getAuthority())
